...
unsigned int id, clock, start_proc, user_tasks, sleeping_tasks;
...
task_t *next_task () // Procura a tarefa mais próxima de acordar e retorna para wakeup
{
    task_t *aux, *task;
    aux = sleeping_queue;
    task = sleeping_queue;
    ...
    if (sleeping_queue) {
        for (int i = 0; i < sleeping_tasks; i++) {
            if (aux->wakeup < aux->next->wakeup) {
                if (aux->wakeup < task->wakeup)
                    task = aux;
            }
            aux = aux->next;
        }
