...
int mqueue_create (mqueue_t *queue, int max_msgs, int msg_size) // Cria fila de mensagens
{
    if (queue && (max_msgs > 0) && (msg_size > 0)) {
        queue->buffer = (void**)malloc(max_msgs*msg_size*sizeof(void*));
        if (queue->buffer) {
            // Cria semáforos
...
int mqueue_send (mqueue_t *queue, void *msg) // Envia mensagem para a fila
{
    if (queue && msg && (queue->status == 1)) {
        // Requisita semáforos
        sem_down (&queue->s_vaga);
        sem_down (&queue->s_buffer);

        // Calcula posição do último elemento
        queue->ultimo_elem = (queue->ultimo_elem + 1) % queue->max_msgs;

        // Copia os bytes da memória de msg para o buffer
        memcpy (&queue->buffer[queue->ultimo_elem], msg, queue->msg_size);
        queue->num_itens++;
...
int mqueue_recv (mqueue_t *queue, void *msg) // Recebe mensagem da fila
{
    if (queue && (queue->status == 1)) {
        ...
        queue->primeiro_elem = (queue->primeiro_elem + 1) % queue->max_msgs;

        // Copia os bytes da memória do buffer para msg
        memcpy (msg, &queue->buffer[queue->primeiro_elem], queue->msg_size);
        queue->num_itens--;
...
int mqueue_destroy (mqueue_t *queue) // Encerra a fila de mensagens
{
    if (queue && (queue->status == 1)) {
        free (queue->buffer);
...
int mqueue_msgs (mqueue_t *queue) // Número de mensagens na fila
{
    if (queue && (queue->status == 1))
        return (queue->num_itens);
    return -1;
}

// /* --------------------------- Teste fila de mensagens com queue.c --------------------------- */
...
//         aux = (void_t*)malloc(sizeof(void_t));

//         // Copia os bytes da memória de msg para aux
//         memcpy (&aux->item, msg, queue->msg_size);

//         // Adiciona aux no buffer
...
//         // Copia os bytes da memória de aux para msg
//         memcpy (msg, &aux->item, queue->msg_size);

//         // Remove aux do buffer
